...
class TestProduction(AssessmentRenderingTestCase):
    ...
    def test_biology(self):
        ...
        assert_that(dom.getElementsByTagName('naquestionbank'), 
                    has_length(1))
        assert_that(dom.getElementsByTagName('naquestionbank')[0], 
                    is_(naquestionbank))

        banks = dom.getElementsByTagName('naquestionbank')
        qset_object = banks[0].assessment_object()
        assert_that(qset_object, has_length(1))
        assert_that(qset_object, 
                    has_property('questions', has_length(1)))
        assert_that(qset_object, 
                    has_property('ntiid', contains_string('set')))

        asg_object = dom.getElementsByTagName('naassignment')[0].assessment_object()
        assert_that(asg_object, 
                    has_property('parts', has_length(1)))
        assert_that(asg_object, 
                    has_property('no_submit', is_false()))
        assert_that(asg_object, 
                    has_property('category_name', is_('no_submit')))
    ...
    def test_ucol(self):
        ...
        dom = _buildDomFromString(_simpleLatexDocument((example,)))
        assert_that(dom.getElementsByTagName('naquestionset'), 
                    has_length(1))
        ...
        assert_that(asg_object, 
                    has_property('category_name', is_('no_submit')))
        assert_that(asg_object,
                    has_property('no_submit', is_true()))
