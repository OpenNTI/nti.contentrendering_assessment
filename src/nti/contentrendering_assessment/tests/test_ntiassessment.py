...
class TestMisc(AssessmentRenderingTestCase):
    ...
    def test_generic_macros(self):
        ...
        values = [u'420',
                  r'\frac{5}{8}',
                  u'\\left(3x+2\\right)\\left(2x+3\\right)',
                  u'\\surd 2',
                  u'\\frac{\\surd \\left(8x+5\\right)\\left(12x+12\\right)}{\\approx 152318}+1204']
        ...
        assert_that(part.hints, 
                    contains(verifiably_provides(IQHint)))
    ...
    def test_file_macros(self):
        ...
        assert_that(part.allowed_mime_types, 
                    contains('application/pdf', 'text/*', '*/*'))
    ...
    def test_essay_macro_with_blank_line_prologue(self):
        ...
        # It's not rendered
        # XXX: This is wrong, should be blank
        assert_that(naq.content,
                    is_("\\label{the.label} Arbitrary content goes here, and there's a blank line, and another before the part: \\begin{naqessaypart} Arbitrary content goes here. \\begin{naqhints} \\naqhint Some hint \\end{naqhints} \\end{naqessaypart}"))
    ...
    def test_question_set_macros(self):
        ...
        assert_that(dom.getElementsByTagName('naquestionset')[0], 
                    is_(naquestionset))
    ...
    def test_assignment_macros(self):
        ...
        assert_that(asg_object.parts[0], 
                    has_property('question_set', same_instance(qset_object)))
        assert_that(asg_object.parts[0], has_property('auto_grade', is_true()))
        assert_that(asg_object.parts[0], 
                    has_property('content', 'Some content.'))
        assert_that(asg_object.parts[0], has_property('title', 'Part Title'))
        # XXX This is wrong. Should just be "Assignment content."
        assert_that(asg_object, 
                    has_property('content', '\\label{assignment} Assignment content. \\begin{naassignmentpart}[auto_grade=true]<Part Title>{set} Some content. \\end{naassignmentpart}'))
    ...
    def test_content_adaptation(self):
        ...
        assert_content("Arbitrary content goes here.",
                       "Arbitrary content goes here.")
        # XXX This is wrong
        assert_content(r"Equation: $123 \times 456\$.",
                       'Equation: \\$123 \\times 456\\\\$ . \\begin{naqsolutions} \\naqsolution Hello \\end{naqsolutions} \\$')
        # XXX: This is wrong too
        assert_content(r"Complex object \begin{tabular}{cc} \\ 1 & 2 \\3 & 4\\ \end{tabular}",
                       'Complex object \\begin{tabular}{cc}1 & 2 \\\\ 3 & 4 \\\\ \\end{tabular}')  # Fails currently
        # XXX: And this one.
        assert_content(r"Figure \begin{figure}[htbp]\begin{center}\includegraphics[width=100px]{images/wu1_square=3=by=x-2.pdf}\end{center}\end{figure}",
                       'Figure \\begin{figure}[htbp]\\begin{center} \\includegraphics [width=100px]{images/wu1_square=3=by=x-2.pdf}\\end{center}\\end{figure}')  # Fails currently
    ...
    def test_free_response_macros(self):
        ...
        assert_that(part, verifiably_provides(part_el.part_interface))
        assert_that(part.content, is_("Arbitrary content goes here."))
        assert_that(part.solutions[0], 
                    has_property("value", "This is a solution. It may require multiple lines. It may span paragraphs."))
    ...
    def test_multiple_choice_macros(self):
        ...
        assert_that(part, verifiably_provides(part_el.part_interface))
        assert_that(part.content, 
                    is_("Arbitrary content for this part goes here."))
        assert_that(part.explanation, 
                    is_("Arbitrary content explaining how the correct solution is arrived at."))
        assert_that(part, has_property('choices', has_length(3)))
        assert_that(part.choices, 
                    has_item('Arbitrary content for the choice.'))

        quest_el = dom.getElementsByTagName('naquestion')[0]
        question = quest_el.assessment_object()
        assert_that(question.content, 
                    is_('Arbitrary prefix content goes here.'))
        assert_that(question.parts, contains(part))
        assert_that(question, 
                    has_property('ntiid', 'tag:nextthought.com,2011-10:testing-NAQ-temp.naq.1'))
    ...
    def test_fill_in_the_blank_short_answer_part(self):
        ...
        naq = dom.getElementsByTagName('naquestion')[0]
        part_el = naq.getElementsByTagName(
            'naqfillintheblankshortanswerpart')[0]
        solns = getattr(part_el, '_asm_solutions')()
        ...
        assert_that(part.solutions, is_(solns))
        assert_that(part, verifiably_provides(part_el.part_interface))
        # XXX This is wrong. Should not have trailing latex
        assert_that(part.content, 
                    is_("Arbitrary content for this part goes here. \\naqblankfield{001}[2] \\naqblankfield{002}[2]"))
        assert_that(part.explanation, 
                    is_("Arbitrary content explaining how the correct solution is arrived at."))
    ...
    def test_fill_in_the_blank_word_bank_question(self):
        ...
        dom = _buildDomFromString(_simpleLatexDocument((example,)))
        assert_that(dom.getElementsByTagName('naquestionfillintheblankwordbank'), 
                    has_length(1))
        ...
        assert_that(assessment, 
                    has_property('ntiid', 'tag:nextthought.com,2011-10:testing-NAQ-temp.naq.1'))
        assert_that(assessment, 
                    externalizes(has_entry('NTIID', assessment.ntiid)))
    ...
    def test_matching_macros(self):
        for part_name in ("naqmatchingpart", "naqorderingpart"):
            ...
            dom = _buildDomFromString(_simpleLatexDocument((example,)))
            assert_that(dom.getElementsByTagName('naquestion'), has_length(1))
            assert_that(dom.getElementsByTagName('naquestion')[0],
                        is_(naquestion))
