...
def _asm_local_sourcecontent(self, ignorable_renderables=()):
    ...
    result = u' '.join(x.source for x in selected_children or ()).strip()
    result = ILatexContentFragment(result)
    return result
...
class _LocalContentMixin(_BaseLocalContentMixin):
    ...
    def _asm_local_content(self):
        if _is_renderable(self.renderer, self.childNodes):
            result = _asm_rendered_textcontent(self, 
...
class _AbstractNonGradableNAQPart(_LocalContentMixin, Base.Environment):
    ...
    def _after_render(self, rendered):
        ...
        # The hints don't normally get rendered# by the templates, so make sure
        # they do
...
class _AbstractNAQPart(_AbstractNonGradableNAQPart):
    ...
    def _asm_solutions(self):
        ...
        for solution_el in solution_els:
            ...
            # TODO: Should this be rendered? In some cases yes, in some cases
            # no?
            content = u' '.join(c.source.strip()
                                for c in solution_el.childNodes).strip()
    ...
    def _asm_explanation(self):
        ...
        return ILatexContentFragment(u'')
    ...
    def _after_render(self, rendered):
        ...
        # The explanations don't normally get rendere by the templates, so make
        # sure they do
